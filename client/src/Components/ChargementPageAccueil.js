...
function Media(props) {
    ...
    return (
        <Grid container wrap="nowrap">
            {
                (loading ? Array.from(new Array(parseInt(props.item))) : data).map((item, index) => (
                    <Box key={index} width={510} marginRight={0.5} my={5}>
                        {item ? (
                            <img style={{ width: 310, height: 118 }} alt={item.title} src={item.src} />
                        ) : (
...
export default function YouTube({ item, width, height }) {
    return (
        <Box overflow="hidden">
            <Media item={item} width={width} height={height} loading />
        </Box>
    );
}